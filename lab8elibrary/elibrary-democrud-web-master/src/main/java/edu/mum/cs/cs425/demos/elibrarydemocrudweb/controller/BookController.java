...
//    @Autowired
    ...
    public ModelAndView searchBooks(@ModelAttribute("book") Book book, @RequestParam(defaultValue = "0") int pageNum) {
        ModelAndView modelAndView = new ModelAndView();
        String sText = book.getTitle();
        System.out.println("sText = " + sText);
        modelAndView.addObject("books", bookService.searchBook(PageRequest.of(pageNum, 3, Sort.by("title")), sText));
        modelAndView.addObject("currentPageNum", pageNum); // yag odoo bga huudasnii dugaar
        modelAndView.setViewName("book/searchResult");
//        System.out.println(bookService.searchBook(PageRequest.of(pageNum, 5, Sort.by("title")));title
        return modelAndView;
    }
}
